...
/*
* on average, 1,024mb is required to generate 50 forms
* above 50, the system returns "Fatal error: Allowed memory size of xxxxx bytes exhausted (tried to allocate xxxx bytes)"
* permit the server to use any amount of memory to generate the forms. 
* ini_set('memory_limit', '-1');
*/

//key variables
$numberOfForms=validate($numberOfForms);

$start=0;
$start=getDetailedTableInfo3("vl_forms_clinicalrequest","year(created)='".getCurrentYear()."' and month(created)='".getCurrentMonth()."'","count(id)","num");
$start+=1;
...
$htmlT="";
for($i=$start;$i<($numberOfForms+$start);$i++) {
	//generate form number
	$formNumber=0;
	$formNumber=generateFormNumber($i,$batchNumber);
		...
		<td style=\"padding:5px\" valign=\"top\"><table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" class=\"vl\">
		  <tr>
			...
			<td width=\"30%\" style=\"padding: 5px; border: 1px solid #333\" valign=\"top\">
				...
				<div><strong>Adult 2nd-Line Regimens:</strong></div>
				<div>
				<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">
				  <tr>
					<td width=\"50%\" valign=\"top\">
...
//filename
$filename=0;
$filename="Clinical.Request.Form.$batchNumber.pdf";
...
//redirect
go("/generateforms/generated/$batchNumber/");
?>