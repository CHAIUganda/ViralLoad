...
$(document).ready(function(){
	...
	form.submit(function()
		{//
		if( validateBatch()   & validateAge() & validateAgeweeks()  & validateMhivstatus() & validateMbfeeding() & validateMdrug() & validateInfantprophylaxis() &validateMentpoint() & validateSpot() & validateSrecstatus()    & validateDatecollected() & validateDateReceived()    )
			return true
		else
			return false;
	});
...
//ensure infant/sample code not null
	...
	//if it's NOT valid
		if(batch.val().length < 1){
			batch.addClass("error");
			batchInfo.text("Please  enter Batch Number!");
			batchInfo.addClass("error");
			return false;
		}
		//if it's valid
		else{
