...
//*******************************************
	...
	function checkUncheckSome(controller,theElements) {
		...
		for(var z=0;z<formElements.length;z++){
			theEnvelope=document.getElementById(formElements[z]);
			if(theEnvelope){
				if(theEnvelope.type){
					if(theEnvelope.type=='checkbox' && theEnvelope.id != theController.id){
						theEnvelope.checked = theController.checked;
					}
				} else {
					var nextArray = '';
					for(var x=0;x <theEnvelope.childNodes.length;x++){
						if(theEnvelope.childNodes[x]){
							if (theEnvelope.childNodes[x].id){
								nextArray += theEnvelope.childNodes[x].id+',';
							}
						}
					}
