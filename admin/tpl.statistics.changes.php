...
switch($soption) {
	...
	?>
		  <tr>
			<td><strong>Data Changes for the date: <font color="#FF0000"><?=getFormattedDate($theDate)?></font></strong></td>
		  </tr>
		  <tr>
			<td><table width="100%"  border="1" class="vl">
			  <tr bgcolor="#ECECFF">
				<td width="74%"><strong>Changes by User</strong> (click to view)</td>
				<td width="26%"><strong>Changes</strong></td>
			  </tr>
		<?
		$squery=0;
		$squery=mysqlquery("select createdby, count(*) stat from vl_logs_tables where date(created)='$theDate' group by createdby order by stat desc");
		...
		?>
			  <tr bgcolor="<?=$scolor?>">
				<td><a href="#" onclick="iDisplayMessage('tpl.statistics.changes.details.php?theDate=<?=$theDate?>&createdby=<?=$sq["createdby"]?>')"><? echo $sq["createdby"]; ?></a></td>
		...
		<? } ?>
			  ...
			  <tr>
				<td colspan="2">&nbsp;[ <a href="../admin/?act=statisticschanges&nav=statistics">return to statistics</a> ]</td>
			  </tr>
			</table></td>
		  </tr>
	<?
	...
	?>
		  <tr>
			<td><strong>Data Changes within the Month of: <font color="#FF0000"><?=$theMonth?>, <?=$theYear?></font></strong></td>
		  </tr>
		  <tr>
			<td><table width="100%"  border="1" class="vl">
			  <tr bgcolor="#ECECFF">
				<td width="74%"><strong>Changes by Day</strong> (click to view)</td>
				<td width="26%"><strong>Changes</strong></td>
			  </tr>
		<?
		$squery=0;
		$squery=mysqlquery("select date(created) datecreated,createdby,count(*) stat from vl_logs_tables where monthname(created)='$theMonth' and year(created)='$theYear' group by date(created) order by created");
		if(mysqlnumrows($squery)) {
			$sq=array();
			$stotal=0;
			$scount=1;
		...
		?>
			  <tr bgcolor="<?=$scolor?>">
				<td><a href="../admin/?act=statisticschanges&nav=statistics&soption=removalsByUser&theDate=<?=$sq["datecreated"]?>"><?=getFormattedDate($sq["datecreated"])?></a></td>
				<td><?=number_format((float)$sq["stat"])?></td>
			  </tr>
		<? 
				$stotal+=$sq["stat"];
...
?>
		  <tr>
			<td><strong>Total Data Changes since February-2012: </strong></td>
		  </tr>
		  <tr>
			<td><strong>Changes per Month: </strong></td>
		  </tr>
		  <tr>
			<td><table width="100%"  border="1" class="vl">
				<tr bgcolor="#ECECFF">
				...
				<?
		$squery=0;
		$squery=mysqlquery("select monthname(created) theMonth, year(created) theYear, count(*) stat from vl_logs_tables group by theMonth, theYear order by created");
		if(mysqlnumrows($squery)) {
			$sq=array();
			$stotal=0;
			$scount=1;
		...
		?>
				<tr bgcolor="<?=$scolor?>">
				  <td><a href="../admin/?act=statisticschanges&nav=statistics&soption=recordsInMonth&theMonth=<?=$sq["theMonth"]?>&theYear=<?=$sq["theYear"]?>"><?=$sq["theMonth"]?>, <?=$sq["theYear"]?></a></td>
				  <td><?=number_format((float)$sq["stat"])?></td>
				</tr>
				<? 
				$stotal+=$sq["stat"];
        ...
        <td style="padding:10px"><table width="100%" border="0" class="vl">
          ...
          <tr>
            <td style="padding:5px">
              <form name="export" method="post" action="tpl.statistics.changes.export.php">
