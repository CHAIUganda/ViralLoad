      ...
      <td width="65%" valign="top">
		<? if($saved) { ?>
            <table width="100%" border="0" class="vl">
                <tr>
                    <td class="vl_success">Facilities Assigned to <?=getDetailedTableInfo2("vl_ips","id='$ipID'","ip")?>!</td>
                </tr>
                <tr>
                    <td>&nbsp;</td>
                </tr>
			...
			case commit:
				...
				if(count($commitRecord)) {
					foreach($commitRecord as $fc) {
						mysqlquery("update vl_facilities set ipID='$ipID' where id='$fc'");
					}
				}
				go("?act=ipsfacilities&ipID=$ipID&nav=configuration&districtID=$districtID&saved=1");
			break;
		}
...
<form action="?act=ipsfacilities&nav=configuration" method="post" name="removeDataForm" id="removeDataForm" onsubmit="return checkForm(this)">
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="vl">
  <tr>
    <td style="padding:0px 0px 5px 0px"><strong>Select Facilities Serviced by <?=getDetailedTableInfo2("vl_ips","id='$ipID'","ip")?></strong></td>
  </tr>
  <tr>
    <td style="padding:5px 0px 5px 0px"><div style="height: 400px; border: 1px solid #ccccff; overflow: auto">
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="trailanalytics">
          <tr>
            ...
            <td class="vl_tdsub" width="40%"><select name="position" id="position" class="search" onchange="document.location.href='?act=ipsfacilities&ipID=<?=$ipID?>&nav=configuration&districtID='+this.value">
						<?
						if($districtID) {
		...
		?>
        <tr>
          <td class="<?=($count<$num?"vl_tdstandard":"vl_tdnoborder")?>"><input name="commitRecord[]" type="checkbox" id="commitRecord[]" value="<?="$q[id]"?>" <?=($q["ipID"]==$ipID?"checked=\"checked\"":"")?>></td>
          <td class="<?=($count<$num?"vl_tdstandard":"vl_tdnoborder")?>">
		  <div><?=$q["facility"]?></div>
          <? if($q["ipID"]) { ?><div class="vls_grey" style="padding:3px 0px">Currently Serviced by: <?=getDetailedTableInfo2("vl_ips","id='$q[ipID]'","ip")?></div><? } ?>
          </td>
  ...
  <tr>
    <td style="padding-top:10px"><input type="submit" name="button" id="button" value="   Assign to <?=getDetailedTableInfo2("vl_ips","id='$ipID'","ip")?>   " /> 
									<input name="commit" type="hidden" id="commit" value="1" />
										<input name="act" type="hidden" id="act" value="ipsfacilities" />
                                        	<input name="option" type="hidden" id="option" value="commit" />
	                                    		<input name="ipID" type="hidden" id="ipID" value="<?=$ipID?>" />
