...
class Calendar {
    ...
    */
    ...
    /*
        check whether this date is in the db
    ...
    */
    function getMonthHTML($m, $y, $showYear = 1) {
    	...
    	$s .= "<tr>\n";
    	$s .= "<td align=\"center\" class=\"caption\" valign=\"top\" width=\"30\">" . (($prevMonth == "") ? "&nbsp;" : "<a href=\"$prevMonth\">&lt;&lt;</a>")  . "</td>\n";
    	$s .= "<td align=\"center\" class=\"caption\" valign=\"top\">$header</td>\n"; 
    	$s .= "<td align=\"center\" class=\"caption\" valign=\"top\" width=\"30\">" . (($nextMonth == "") ? "&nbsp;" : "<a href=\"$nextMonth\">&gt;&gt;</a>")  . "</td>\n";
    	$s .= "</tr>";
		$s .= "</table>\n";
    	
    	$s .= "<table id=\"calendar\" cellspacing=\"0\" cellpadding=\"0\" summary=\"This month's calendar\" width=\"200\">\n";
    	$s .= "<tr>\n";
    	$s .= "<th scope=\"col\" abbr=\"Sunday\" title=\"Sunday\">" . $this->dayNames[($this->startDay)%7] . "</th>\n";
    	$s .= "<th scope=\"col\" abbr=\"Monday\" title=\"Monday\">" . $this->dayNames[($this->startDay+1)%7] . "</th>\n";
    	$s .= "<th scope=\"col\" abbr=\"Tuesday\" title=\"Tuesday\">" . $this->dayNames[($this->startDay+2)%7] . "</th>\n";
    	$s .= "<th scope=\"col\" abbr=\"Wednesday\" title=\"Wednesday\">" . $this->dayNames[($this->startDay+3)%7] . "</th>\n";
    	$s .= "<th scope=\"col\" abbr=\"Thursday\" title=\"Thursday\">" . $this->dayNames[($this->startDay+4)%7] . "</th>\n";
    	$s .= "<th scope=\"col\" abbr=\"Friday\" title=\"Friday\">" . $this->dayNames[($this->startDay+5)%7] . "</th>\n";
    	$s .= "<th scope=\"col\" abbr=\"Saturday\" title=\"Saturday\">" . $this->dayNames[($this->startDay+6)%7] . "</th>\n";
    	$s .= "</tr>\n";
    	...
    	while ($d <= $daysInMonth) {
			...
			for ($i = 0; $i < 7; $i++) {
				...
				$class = 0;
				$class = (($year == $today["year"] && $month == $today["mon"] && $d == $today["mday"])? "today" : "td");
				
				if($this->isDateInDB($mydate)) {
					$class=0;
					$class="todayE";
				}
...
class MyCalendar extends Calendar {
    function getCalendarLink($month, $year) {
		...
		return "index.php?act=today&month=$month&year=$year"; 
	}

    function getDateLink($day, $month, $year) {
        //link based on db
        $link = 0;
		$link = "index.php?a=calendar&act=calendar&day=$day&month=$month&year=$year&showeditor=1";
        return $link;
    }
}
?>