        ...
        switch($option) {
            case add:
				...
				if(!$error) {
					...
					if(!isQuery("select * from vl_appendix_treatmentinitiation where appendix='$appendix'")) {
...
?>
        ...
        <form name="appendicesForm" method="post" action="?act=atreatmentinitiation&nav=configuration" onsubmit="return checkForm(this)">
        ...
        <? } ?>
            <tr> 
              <td style="padding:10px 0px 10px 0px"><table width="100%" border="0" class="vl">
                <tr>
                  ...
                  <td width="70%"><input type="text" name="appendix" id="appendix" class="search" size="25" value="<?=($id?getDetailedTableInfo2("vl_appendix_treatmentinitiation","id='$id'","appendix"):"")?>"></td>
                </tr>
                <tr>
                  <td>Position</td>
                  <td><select name="position" id="position" class="search">
						...
						$position=0;
						if(!$id) {
							$position=getDetailedTableInfo3("vl_appendix_treatmentinitiation","appendix!=''","count(id)","num");
							$position+=1;
						} else {
							$position=getDetailedTableInfo2("vl_appendix_treatmentinitiation","id='$id'","position");
						}
            ...
            <tr> 
              ...
              <? if($task=="modify") { ?>
              <button type="button" id="button" name="button" value="button" onclick="document.location.href='?act=atreatmentinitiation&nav=configuration'">&nbsp;&nbsp;Cancel&nbsp;&nbsp;</button>
              <input name="id" type="hidden" id="id" value="<?=$id?>"> 
              <? } ?>
              <input name="act" type="hidden" id="act" value="atreatmentinitiation">
                ...
                <td style="padding:5px 0px" align="center">
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" class="vl">
                        ...
                        while($q=mysqlfetcharray($query)) {
                            $count+=1;
							$numberSamples=0;
							$numberSamples=getDetailedTableInfo3("vl_samples","treatmentInitiationID='$q[id]'","count(id)","num");
                        ?>
                            <tr>
                                ...
                                <td class="<?=($count<$num?"vl_tdstandard":"vl_tdnoborder")?>"><a href="?act=atreatmentinitiation&nav=configuration&modify=modify&id=<?=$q["id"]?>">edit</a><? if(!$numberSamples) { ?>&nbsp;::&nbsp;<a href="javascript:if(confirm('Are you sure?')) { document.location.href='?act=atreatmentinitiation&nav=configuration&option=remove&id=<?=$q["id"]?>'; }">delete</a><? } ?></td>
                            </tr>
                        <? } ?>
                    </table>
                    </div>
        ...
        <table border="0" align="center" cellpadding="0" cellspacing="0" style="border:1px solid #d5d5d5" width="100%">
          <tr>
            <td style="padding:10px"><table width="100%" border="0" class="vl">
              <tr>
                <td><strong>MANAGE TREATMENT INITIATION</strong></td>
              </tr>
              <tr>
                <td>&nbsp;</td>
              </tr>
