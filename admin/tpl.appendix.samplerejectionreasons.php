                ...
                <td style="padding:5px 0px" align="center">
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0" class="vl">
                        ...
                        while($q=mysqlfetcharray($query)) {
                            $count+=1;
							$numberSamples=0;
							$numberSamples=getDetailedTableInfo3("vl_samples_verify","outcomeReasonsID='$q[id]'","count(id)","num");
                        ?>
                            <tr>
                                ...
                                <td class="<?=($count<$num?"vl_tdstandard":"vl_tdnoborder")?>"><a href="?act=asamplerejectionreasons&nav=configuration&modify=modify&id=<?=$q["id"]?>">edit</a><? if(!$numberSamples) { ?>&nbsp;::&nbsp;<a href="javascript:if(confirm('Are you sure?')) { document.location.href='?act=asamplerejectionreasons&nav=configuration&option=remove&id=<?=$q["id"]?>'; }">delete</a><? } ?></td>
                            </tr>
                        <? } ?>
                    </table>
                    </div>
