						...
						*/
						if(!$error) {
							...
							for($i=2;$i<=$excelData->sheets[0]['numRows'];$i++) {
								//Form Number
								$formNumber=0;
								$formNumber=validate(trim($excelData->sheets[0]['cells'][$i][1]));
								
								//Sample ID
								$vlSampleID=0;
								$vlSampleID=validate(trim($excelData->sheets[0]['cells'][$i][3]));
								
								//Facility
								$facilityID=0;
								$facilityID=logNewFacility(validate(trim($excelData->sheets[0]['cells'][$i][4])),validate(trim($excelData->sheets[0]['cells'][$i][5])));
								
								//District
								$districtID=0;
								$districtID=logNewDistrict(validate(trim($excelData->sheets[0]['cells'][$i][5])));

								//Hub
								$hubID=0;
								$hubID=logNewHub(validate(trim($excelData->sheets[0]['cells'][$i][6])),$facilityID);

								//Date of Collection
								$collectionDate=0;
								$collectionDate=getFormattedDateCRBExcel(validate(trim($excelData->sheets[0]['cells'][$i][7])));
								
								//Sample Type
								$sampleTypeID=0;
								$sampleTypeID=getDetailedTableInfo2("vl_appendix_sampletype","lower(appendix)='".strtolower(validate(trim($excelData->sheets[0]['cells'][$i][7])))."' limit 1","id");
								
								//patientID
								$patientID=0;
								$patientID=getDetailedTableInfo2("vl_samples","vlSampleID='".validate(trim($excelData->sheets[0]['cells'][$i][3]))."' limit 1","patientID");
								
								//Patient ART
								updatePatientData($patientID,"artNumber",validate(trim($excelData->sheets[0]['cells'][$i][9])),$collectionDate);

								//Patient OtherID
								updatePatientData($patientID,"otherID",validate(trim($excelData->sheets[0]['cells'][$i][10])),$collectionDate);

								//Gender
								updatePatientData($patientID,"gender",validate(trim($excelData->sheets[0]['cells'][$i][11])),$collectionDate);

								//Age (Years)
								updatePatientData($patientID,"dateOfBirth",validate(trim($excelData->sheets[0]['cells'][$i][12])),$collectionDate);

								//Phone Number
								logPatientPhone($patientID,validate(trim($excelData->sheets[0]['cells'][$i][13])));
								
								//Has Patient Been on treatment for at least 6 months
								$treatmentLast6Months=0;
								$treatmentLast6Months=validate(trim($excelData->sheets[0]['cells'][$i][14]));

								//Date of Treatment Initiation
								$treatmentInitiationDate=0;
								$treatmentInitiationDate=getRawFormattedDateLessDay(validate(trim($excelData->sheets[0]['cells'][$i][15])));

								//Current Regimen
								$currentRegimenID=0;
								$currentRegimenID=logNewCurrentRegimen(validate(trim($excelData->sheets[0]['cells'][$i][16])));

								//Indication for Treatment Initiation
								$treatmentInitiationID=0;
								$treatmentInitiationID=logNewTreatmentInitiation(validate(trim($excelData->sheets[0]['cells'][$i][17])));

								//Which Treatment Line is Patient on
								$treatmentStatusID=0;
								$treatmentStatusID=logNewTreatmentStatus(validate(trim($excelData->sheets[0]['cells'][$i][18])));

								//Reason for Failure
								$reasonForFailureID=0;
								$reasonForFailureID=logNewReasonForFailure(validate(trim($excelData->sheets[0]['cells'][$i][19])));

								//Is Patient Pregnant
								$pregnant=0;
								$pregnant=validate(trim($excelData->sheets[0]['cells'][$i][20]));

								//ANC Number
								$pregnantANCNumber=0;
								$pregnantANCNumber=validate(trim($excelData->sheets[0]['cells'][$i][21]));

								//Is Patient Breastfeeding
								$breastfeeding=0;
								$breastfeeding=validate(trim($excelData->sheets[0]['cells'][$i][22]));

								//Patient has Active TB
								$activeTBStatus=0;
								$activeTBStatus=validate(trim($excelData->sheets[0]['cells'][$i][23]));

								//If Yes are they on
								$tbTreatmentPhaseID=0;
								$tbTreatmentPhaseID=logNewTBTreatmentPhase(validate(trim($excelData->sheets[0]['cells'][$i][24])));

								//ARV Adherence
								$arvAdherenceID=0;
								$arvAdherenceID=logNewARVAdherence(validate(trim($excelData->sheets[0]['cells'][$i][25])));

								//Routine Monitoring
								$vlTestingRoutineMonitoring=0;
								$vlTestingRoutineMonitoring=validate(trim($excelData->sheets[0]['cells'][$i][26]));

								//Last Viral Load Date
								$routineMonitoringLastVLDate=0;
								$routineMonitoringLastVLDate=(trim($excelData->sheets[0]['cells'][$i][27])?getRawFormattedDateLessDay(validate(trim($excelData->sheets[0]['cells'][$i][27]))):"");

								//Value
								$routineMonitoringValue=0;
								$routineMonitoringValue=(trim($excelData->sheets[0]['cells'][$i][28])?getRawFormattedDateLessDay(validate(trim($excelData->sheets[0]['cells'][$i][28]))):"");

								//Sample Type
								$routineMonitoringSampleTypeID=0;
								$routineMonitoringSampleTypeID=getDetailedTableInfo2("vl_appendix_sampletype","lower(appendix)='".strtolower(validate(trim($excelData->sheets[0]['cells'][$i][29])))."' limit 1","id");

								//Repeat Viral Load Test after Suspected Treatment Failure adherence counseling
								$vlTestingRepeatTesting=0;
								$vlTestingRepeatTesting=validate(trim($excelData->sheets[0]['cells'][$i][30]));

								//Last Viral Load Date
								$repeatVLTestLastVLDate=0;
								$repeatVLTestLastVLDate=(trim($excelData->sheets[0]['cells'][$i][31])?getRawFormattedDateLessDay(validate(trim($excelData->sheets[0]['cells'][$i][31]))):"");

								//Value
								$repeatVLTestValue=0;
								$repeatVLTestValue=validate(trim($excelData->sheets[0]['cells'][$i][32]));

								//Sample Type
								$repeatVLTestSampleTypeID=0;
								$repeatVLTestSampleTypeID=getDetailedTableInfo2("vl_appendix_sampletype","lower(appendix)='".strtolower(validate(trim($excelData->sheets[0]['cells'][$i][33])))."' limit 1","id");

								//Suspected Treatment Failure
								$vlTestingSuspectedTreatmentFailure=0;
								$vlTestingSuspectedTreatmentFailure=validate(trim($excelData->sheets[0]['cells'][$i][34]));

								//Last Viral Load Date
								$suspectedTreatmentFailureLastVLDate=0;
								$suspectedTreatmentFailureLastVLDate=(trim($excelData->sheets[0]['cells'][$i][35])?getRawFormattedDateLessDay(validate(trim($excelData->sheets[0]['cells'][$i][35]))):"");

								//Value
								$suspectedTreatmentFailureValue=0;
								$suspectedTreatmentFailureValue=validate(trim($excelData->sheets[0]['cells'][$i][36]));

								//Sample Type
								$suspectedTreatmentFailureSampleTypeID=0;
								$suspectedTreatmentFailureSampleTypeID=getDetailedTableInfo2("vl_appendix_sampletype","lower(appendix)='".strtolower(validate(trim($excelData->sheets[0]['cells'][$i][37])))."' limit 1","id");
							...
							for($i=2;$i<=$excelData->sheets[1]['numRows'];$i++) {
								//Facility
								$facilityID=0;
								$facilityID=logNewFacility(validate(trim($excelData->sheets[1]['cells'][$i][1])),validate(trim($excelData->sheets[1]['cells'][$i][2])));
								
								//District
								$districtID=0;
								$districtID=logNewDistrict(validate(trim($excelData->sheets[1]['cells'][$i][2])));

								//Hub
								$hubID=0;
								$hubID=logNewHub(validate(trim($excelData->sheets[1]['cells'][$i][3])),$facilityID);

								//IP
								$ipID=0;
								$ipID=logNewIP(validate(trim($excelData->sheets[1]['cells'][$i][4])),$facilityID,$hubID);
							}
							...
							for($i=2;$i<=$excelData->sheets[2]['numRows'];$i++) {
								//Current Regimen
								$currentRegimenID=0;
								$currentRegimenID=logNewCurrentRegimen(validate(trim($excelData->sheets[2]['cells'][$i][1])));

								//Which Treatment Line is Patient on
								$treatmentStatusID=0;
								$treatmentStatusID=logNewTreatmentStatus(validate(trim($excelData->sheets[2]['cells'][$i][2])));
