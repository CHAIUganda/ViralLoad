...
class DOMPDF {
  ...
  private function write_log() {
    ...
    $memory = number_format((float)$memory/1024);
    $time = number_format((float)(microtime(true) - $this->_start_time) * 1000, 2);
  ...
  /**
   * output the file to the disk
   * @param string $filename the name of the streamed file
   * @param array  $options header options (see above)
   */
  function outputToFile($filename, $options = null) {
    $this->save_locale();
	
    $this->write_log();
    
    if(!is_null($this->_pdf))
      $this->_pdf->outputToFile($filename, $options);
