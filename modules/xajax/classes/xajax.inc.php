...
///////////////////////////////////////////////////////////////////////////////
// vlDC version 0.1 beta4
// copyright (c) 2005 by J. Max Wilson
// http://vlDC.sourceforge.net
//
//
// vlDC is an open source PHP class library for easily creating powerful
// PHP-driven, web-based AJAX Applications. Using vlDC, you can asynchronously
// call PHP functions and update the content of your your webpage without
// reloading the page.
//
// vlDC is released under the terms of the LGPL license
...
// but WITHOUT ANY WARRANTY; without even the implied warrvly of
...
// The vlDCResponse class is used to created responses to be sent back to your
// webpage.  A response contains one or more command messages for updating your page.
// Currently vlDC supports five kinds of command messages:
...
class vlDCResponse
{
	...
	// Constructor
	function vlDCResponse()
	{
		/**
		* changed from UTF-8 to iso-8859-1 encoding since it was failing
		* to return characters such as: é
	...
	// $sData is the data you wvl to set the attribute to
	...
	// $sTarget is a string containing the id of an HTML element
	// $sAttribute is the part of the element you wish to modify ("innerHTML", "value", etc.)
	// $sData is the data you wvl to append to the end of the attribute
	...
	// $sTarget is a string containing the id of an HTML element
	// $sAttribute is the part of the element you wish to modify ("innerHTML", "value", etc.)
	// $sData is the data you wvl to prepend to the beginning of the attribute
	...
	// addScript() adds a vlript command message to your xml response
	...
	{
		$this->xml .= "<vlript><![CDATA[$sJS]]></vlript>";
	}
	...
	// getXML() returns the xml to be returned from your function to the vlDC
	// processor on your page
	// usage: $objResponse->getXML();
	function getXML()
	{
		if (strstr($this->xml,"</vlDC>") == false)
			$this->xml .= "</vlDC>";
		
		return $this->xml; 
	}
}// end class vlDCResponse
...
// the vlDC class generates the vlDC javascript for your page including the 
// javascript wrappers for the PHP functions that you wvl to call from your page.
...
class vlDC
{
	...
	var $sRequestURI;			// The URI for making requests to the vlDC object
	...
	// $sWrapperPrefix - defaults to "vlDC_";
	// usage: $vlDC = new vlDC();
	function vlDC($sRequestURI="",$sWrapperPrefix="vlDC_",$bDebug=false)
	{
	...
	function detectURI()
	{
        ...
        if (empty($aUri['scheme']))
		{
			...
			{
				print "vlDC Error: vlDC failed to automatically identify your Request URI.";
				print "Please set the Request URI explicitly when you instvliate the vlDC object.";
                exit();
            }
	...
	// usage: $vlDC->setRequestURI("http://vlDC.sourceforge.net");
	function setRequestURI($sRequestURI)
	{
		$this->sRequestURI = $sRequestURI;
	}
	
	// debugOn() enables debug messages for vlDC
	// usage: $vlDC->debugOn();
	function debugOn()
	{
		$this->bDebug = true;
	...
	// debugOff() disables debug messages for vlDC
	// usage: $vlDC->debugOff();
	function debugOff()
	{
		$this->bDebug = false;
	...
	// statusMessagesOn() enables messages in the statusbar for vlDC
	// usage: $vlDC->statusMessagesOn();
	function statusMessagesOn()
	{
		$this->bStatusMessages = true;
	}
	
	// statusMessagesOff() disables messages in the statusbar for vlDC
	// usage: $vlDC->statusMessagesOff();
	...
	{
		trigger_error("vlDC: the <b>addFunction()</b> method has been renamed <b>registerFunction()</b>. <br />Please use ->registerFunction('$sFunction'".($sRequestType==GET?",GET":"")."); instead.",E_USER_WARNING);
		$this->registerFunction($sFunction,$sRequestType);
	}
	
	// registerFunction() registers a PHP function to be callable through vlDC
	...
	// usage: $vlDC->registerFunction("myfunction",POST);
	...
	{
		$js = "function ".$this->sWrapperPrefix."$sFunction(){vlDC.call(\"$sFunction\", arguments, ".$sRequestType.");}\n";		
		return $js;
	}
	
	// processRequests() is the main communications engine of vlDC
	// The engine handles all incoming vlDC requests, calls the apporiate PHP functions
	...
	// xmlToArray() takes a string containing vlDC xjxobj xml or xjxquery xml
	...
	{
		trigger_error("vlDC: the <b>javascript()</b> method has been renamed <b>printJavascript()</b>. <br />Please use ->printJavascript(".($sJsURI==""?"":"'$sJsURI'")."); instead.",E_USER_WARNING);
		$this->printJavascript($sJsURI);
	}
	
	// printJavascript() prints the vlDC javascript code into your page
	// it should only be called between the <head> </head> tags
	// usage:
	//	<head>
	//		...
	//		<?php $vlDC->printJavascript(); 
	function printJavascript($sJsURI="")
	{
		print $this->getJavascript($sJsURI);
	}
	
	// getJavascript() returns the vlDC javascript code that should be added to
	// your page between the <head> </head> tags
	// usage:
	//	<head>
	//		...
	//		<?php $vlDC->getJavascript(); 
	...
	{	
		...
		$html  = "<script type=\"text/javascript\">var vlDCRequestUri=\"".$this->sRequestURI."\";</script>\n";
		$html .= "\t<script type=\"text/javascript\" src=\"".$sJsURI.$separator."vlDCjs=vlDCjs\"></script>\n";
	...
	// generateJavascript() generates all of the vlDC javascript code including the javascript
	...
	{
		$js  = "";
		if ($this->bDebug){ $js .= "var vlDCDebug=".($this->bDebug?"true":"false").";\n"; }
		
		ob_start();
		?>
		function vlDC()
		{
			<?php if ($this->bDebug){ ?>this.DebugMessage = function(text){if (vlDCDebug) alert("vlDC Debug:\n " + text)}<?php	} ?>
			
			this.workId = 'vlDCWork'+ new Date().getTime();
			this.depth = 0;
			...
			this.getRequestObject = function()
			{
				...
				if(!req && typeof XMLHttpRequest != "undefined")
					...
					<?php if ($this->bDebug){ ?>if (!req) this.DebugMessage("Request Object Instvliation failed.");<?php } ?>
					
				return req;
			}

			// vlDC.$() is shorthand for document.getElementById()
			this.$ = function(sId)
			{
				return document.getElementById(sId);
			}
			
			// vlDC.getFormValues() builds a query string XML message from the elements of a form object
			...
			// Generates an XML message that vlDC can understand from a javascript object
			...
			this.call = function(sFunction, aArgs, sRequestType)
			{
				...
				<?php if ($this->bDebug){ ?>this.DebugMessage("Starting vlDC...");<?php } ?>
				var vlDCRequestType = sRequestType;
				var uri = vlDCRequestUri;
				var value;
				switch(vlDCRequestType)
				{
					case <?php print GET; ?>:{
						var uriGet = uri.indexOf("?")==-1?"?vlDC="+encodeURIComponent(sFunction):"&vlDC="+encodeURIComponent(sFunction);
						for (i = 0; i<aArgs.length; i++)
						{
							value = aArgs[i];
							if (typeof(value)=="object")
								value = this.objectToXML(value);
							uriGet += "&vlDCargs[]="+encodeURIComponent(value);
						}
						uriGet += "&vlDCr=" + new Date().getTime();
						uri += uriGet;
						postData = null;
						} break;
					case <?php print POST; ?>:{
						postData = "vlDC="+encodeURIComponent(sFunction);
						postData += "&vlDCr="+new Date().getTime();
						for (i = 0; i <aArgs.length; i++)
						{
							value = aArgs[i];
							if (typeof(value)=="object")
								value = this.objectToXML(value);
							postData = postData+"&vlDCargs[]="+encodeURIComponent(value);
						}
						} break;
					default:
						alert("Illegal request type: " + vlDCRequestType); return false; break;
				}
				r = this.getRequestObject();
				r.open(vlDCRequestType==<?php print GET; ?>?"GET":"POST", uri, true);
				if (vlDCRequestType == <?php print POST; ?>)
				{
					try
					{
				...
				r.onreadystatechange = function()
				{
					...
					if (r.status==200)
					{
						<?php if ($this->bDebug){ ?>vlDC.DebugMessage("Received:\n" + r.responseText);<?php } ?>
						var data = r.responseXML;
						if (data)
							vlDC.processResponse(data);
					}
				}
			...
			// Tests if the new Data is the same as the extvl data
			this.willChange = function(element, attribute, newData)
			{
				var oldData;
				if (attribute == "innerHTML")
				{
					tmpvlDC = this.$(this.workId);
					if (tmpvlDC == null)
					{
						tmpvlDC = document.createElement("div");
						tmpvlDC.setAttribute('id',this.workId);
						tmpvlDC.style.display = "none";
						tmpvlDC.style.visibility = "hidden";
						document.body.appendChild(tmpvlDC);
					}
					tmpvlDC.innerHTML = newData;
					newData = tmpvlDC.innerHTML;
				}
			...
			//Process XML vlDCResponses returned from the request
			this.processResponse = function(xml)
			{
				...
				for (i=0; i<xml.childNodes.length; i++)
				{
					...
					if (xml.childNodes[i].nodeName == "vlript")
					{
						if (xml.childNodes[i].firstChild)
